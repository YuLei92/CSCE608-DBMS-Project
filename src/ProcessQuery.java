...
public class ProcessQuery {
    ...
    public void process(String query_){
        ...
        if(parser.keyword.get(0).equals("create")){
            op_create();
        }else if(parser.keyword.get(0).equals("drop")){
            op_drop();
        }else if(parser.keyword.get(0).equals("insert")){
            op_insert();
        }
    }

    //realize insert without select
    //Insert需要完成的： 1. 对block方面进行优化，不能随意采用制定的block 2.集成select语句
